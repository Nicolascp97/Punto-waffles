...
import {
  WAFFLE_PHONE,
  FRUITS, BASES, SAUCES, DECORATIONS,
  ICE_CREAMS_CREMA, ICE_CREAMS_AGUA, ICE_CREAMS_ZERO,
  WAFFLES_SALADO,
} from '../../game-data/menu'
...
export default function StepComplete({ order, onReset }) {
  ...
  useEffect(() => {
    if (!fired.current) {
      ...
      confetti({
        ...
        origin: { y: 0.5 },
        colors: isSalado
          ? ['#D4006A', '#880E4F', '#F9A8D4', '#ffffff']
          : ['#FF6B2C', '#FFB347', '#22c55e', '#ff6b8a'],
      })
    }
  }, [])
  ...
  // ── SALADO ticket rows ────────────────────────────────────
  const salaWaffleLabel = isSalado ? getLabel(WAFFLES_SALADO, order.salaWaffle) : null
  const salaWaffleData  = isSalado ? WAFFLES_SALADO.find(w => w.id === order.salaWaffle) : null
  ...
  ].filter(r => r.val !== '—' && r.val !== 'Sin salsa' && r.val !== 'Sin topping' && r.val !== 'Sin Helado' && r.val !== 'No')

  // ── WhatsApp messages ─────────────────────────────────────
  const whatsappMsg = encodeURIComponent(isSalado
    ? (
      `[NUEVA COMANDA: ${orderId.current}] 🔔\n` +
      `---------------------------\n` +
      `TIPO: 🧀 Salado\n` +
      `WAFFLE: ${salaWaffleLabel}\n` +
      `ENTREGA: ${order.delivery === 'takeaway' ? '🥡 Para Llevar' : '🍽️ En Local'}\n` +
      `---------------------------\n` +
      (salaWaffleData ? `Ingredientes: ${salaWaffleData.ingredients.join(', ')}\n` : '') +
      `---------------------------\n` +
    ...
    : (
      ...
      `ENTREGA: ${order.delivery === 'takeaway' ? '🥡 Para Llevar' : '🍽️ En Local'}\n` +
      `---------------------------\n` +
      dulceRows.map(r => `[_] ${r.label.toUpperCase()}: ${r.val}`).join('\n') + '\n' +
      `---------------------------\n` +
      `¡Enviado desde el Menú Interactivo! ✨`
    )
  )
  ...
  return (
    <div className="space-y-6 lg:space-y-8 bg-white border-2 border-[var(--border)] rounded-[var(--radius-xl)] p-5 sm:p-10 shadow-lg">
      ...
      {/* Thermal Ticket */}
      <div
        className="bg-[#fffdf9] rounded-b-md p-6 sm:p-8 border border-[#e5e0d8] relative shadow-sm max-w-sm mx-auto"
        style={{
      ...
      >
        <div className="text-center border-b-2 border-dashed border-[#d1c8bc] pb-4 mb-5">
          <h3 className="font-bold text-[20px] uppercase tracking-[0.2em] text-[#5c5346] font-mono">Punto Waffles</h3>
          <p className="text-[12px] uppercase text-[#8c8273] font-mono mt-1 tracking-widest">
            Comanda Cocina {orderId.current}
          </p>
        </div>

        <ul className="space-y-3 font-mono text-[#4a4236] text-[13px] sm:text-[14px]">
          <li className="flex justify-between font-bold mb-4 uppercase tracking-wide">
            <span>TIPO:</span>
            <span>{isSalado ? '🧀 Salado' : '🧇 Dulce'}</span>
          </li>
          ...
          {/* Salado: show selected waffle */}
          {isSalado && salaWaffleData && (
            <>
              <li className="flex flex-col leading-snug border-b border-dotted border-[#e5e0d8] pb-3 mb-3">
                <span className="uppercase tracking-wide font-bold mb-1">
                  ☐ WAFFLE: {salaWaffleLabel}
                </span>
                <div className="flex flex-wrap gap-1 mt-1">
                  {salaWaffleData.ingredients.map(ing => (
                    <span key={ing} className="text-[10px] bg-[#f4ecd8] px-1.5 py-0.5 rounded-sm font-medium">
                      {ing}
                    </span>
                  ))}
                </div>
          ...
          {/* Dulce: show all selected options */}
          {!isSalado && dulceRows.map((row, i) => (
            <li key={i} className="flex flex-col sm:flex-row sm:items-end justify-between leading-snug">
              ...
              <span className="border-b-2 border-dotted border-[#e5e0d8] flex-1 mx-3 hidden sm:block mb-1" />
        ...
        <div className="text-center border-t-2 border-dashed border-[#d1c8bc] pt-5 mt-6">
      ...
      <div className="space-y-4 pt-4 max-w-sm mx-auto">
        <a
          ...
          className="btn-primary w-full text-[15px] sm:text-[16px] py-4 shadow-lg hover:shadow-xl flex items-center justify-center gap-3"
        ...
        <button onClick={onReset} className="btn-secondary w-full text-[14px]">
          Volver a empezar
        </button>
      </div>
    </div>
